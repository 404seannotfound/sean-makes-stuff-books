#!/usr/bin/env python3
"""Build a web-safe novel.html for sean-makes-stuff/books/fortune-cookie/.

Reads from the scriptorium source but writes with .jpg paths and
sean-makes-stuff CSS, matching the cant-have-nice-things web layout.
"""
import json, re
...
TITLE = "The Empty Jar"
SUBTITLE = "a fortune-cookie novel · woven from eighteen chapters"
...
def load_chapter(n):
    ...
    # Manifest is a flat list of beat dicts
    beats_data = json.loads((SRC / f"chapters/{nn}/images/beats.json").read_text())
    beat_details = beats_data.get("beats", [])
    beats = []
    for i, m in enumerate(manifest):
        b = beat_details[i] if i < len(beat_details) else {}
        beats.append({
            "filename": m["file"],
            "scene_name": m.get("scene_name") or b.get("scene_name", ""),
            "page_position": m.get("position") or b.get("page_position", "midchapter"),
            "mood": m.get("mood") or b.get("mood", ""),
            "weight": m.get("weight") or b.get("weight", ""),
        })
...
def render_chapter(ch, is_first):
    ...
    out.append(f'  <div class="ch-head"><div class="ch-num">Chapter {n}</div><h2 class="ch-title">{escape(ch["title"])}</h2></div>')
    ...
    for i, para in enumerate(ch["paragraphs"]):
        ...
        if i in insertions:
            for b in insertions[i]:
                fname = b["filename"].replace(".jpeg", ".jpg").replace(".png", ".jpg")
...
chapters = [load_chapter(n) for n in range(1, 19)]
...
# Audiobook label — 202 min, 18 markers
AUDIO_LABEL = "🎧 Full audiobook · 3h 22m · 10 voices · 18 chapter markers"
...
<meta name="description" content="A middle-grade illustrated novel in 18 chapters with 143 hand-drawn Inksmoke illustrations and a three-hour audiobook narrated with 10 distinct voices.">
<style>{CSS}</style>
</head>
<body>
<nav class="book-nav">
  <span class="label">{escape(TITLE)} · {total_words:,} words · {total_beats} illustrations · 3h 22m audio</span>
...
<main class="book">
  ...
  <div class="audio-hero">
    ...
    <a class="audio-hero__download" href="audio/audiobook.mp3" download>⬇ Download MP3<small>~97 MB · open in a podcast app to see chapters</small></a>
  </div>
...
{chapters_html}
  ...
  <section class="back-matter">
    <p>An illustrated novel. Read it. Hear it.</p>
  </section>
</main>
</body>
</html>
